...
desired_sdk = "E:\\nugetcache\\androidsdk.29.0.1" # os.getenv("ANDROID_SDK_ROOT")
if not desired_sdk:
...
desired_jdk = "E:\\nugetcache\\jdk.1.8.3\\x64" #os.getenv("JAVA_HOME")
if not desired_jdk:  
...
def list_dependencies(app_dir, output_file_path):
...
#    gradle_command = "./gradlew :hubapphost:dependencies --configuration implementation"    
    gradle_command = "gradlew.bat :hubapphost:dependencies --configuration implementation"
...
def main():
    if len(sys.argv) == 2:
        hubapphost_dir = sys.argv[1]
    else:
        hubapphost_dir = os.getcwd()

    # Some smoke checks to ensure that we have a valid hubapphost checkout and inside android folder.
    packageJsonFile = os.path.join(hubapphost_dir, "package.json")
    with open(packageJsonFile) as packageJsonText:
        packageJson = json.load(packageJsonText)
        if(packageJson[u'name'] != u'HubAppHost' ):
            logging.info("Not a valid HubAppHost repo path!")
            exit(-1)

    hubapphost_dir = os.path.join(hubapphost_dir, "android")
    os.chdir(hubapphost_dir)

    dependency_dir_root = os.path.join(hubapphost_dir, "dependencies")
    ...
    log_file_path = os.path.join(hubapphost_dir, "log_" + time.strftime("%Y%m%d-%H%M%S") + ".txt" )

    if(not os.path.exists(os.path.join(hubapphost_dir, "dependencies"))):
        os.mkdir(os.path.join(hubapphost_dir, "dependencies"))

    logging.basicConfig(level = logging.DEBUG, filename = log_file_path)
    logging.info("hubapphost_dir: " + hubapphost_dir)
    ...
    list_dependencies(hubapphost_dir, dependency_list_file_path)
